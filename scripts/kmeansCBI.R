...
model <- kmeansCBI(gifts[, 2:3, with=FALSE], k=clstrs, iter.max = 10000, 
                   scaling=TRUE)
gifts[, TripId := model$partition]

giftcounts <- gifts[, .N, by=TripId]
solos <- giftcounts[N==1, TripId]
sapply(solos, function(solo) gifts[TripId==solo, TripId := clstrs+1])

#duos <- giftcounts[N==2, TripId]
#sapply(duos, function(duos) gifts[TripId==duos, TripId := clstrs+2])

setorder(gifts, -Weight)
submission <- as.data.frame(gifts[, .(GiftId, TripId)])
...
trips <- merge(base_data, submission)
tripSums <- aggregate(Weight~TripId, data=trips, FUN=sum)
nrow(tripSums[tripSums$Weight > 990, ])

write.csv(submission,file="../submissions/kmeansCBI_5280_1000.csv",row.names=FALSE)
